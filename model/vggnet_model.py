...
def vgg_16(inputs,
  ...
  with tf.variable_scope(scope, 'vgg_16', [inputs]) as sc:
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
      ...
      net = slim.repeat(
              net, 1, slim.conv2d, 64, [3, 3], scope='conv1_final', **kwargs)
      net = slim.max_pool2d(net, [2, 2], scope='pool1')
      net = slim.repeat(
              net, 1, slim.conv2d, 128, [3, 3], scope='conv2')
      net = slim.repeat(
              net, 1, slim.conv2d, 128, [3, 3], scope='conv2_final', **kwargs)
      net = slim.max_pool2d(net, [2, 2], scope='pool2')
      net = slim.repeat(
      ...
      # Use conv2d instead of fully_connected layers.
      net = slim.conv2d(net, 4096, [7, 7], padding=fc_conv_padding, scope='fc6', **kwargs)
      if dropout_keep_prob > 0:
        net = slim.dropout(net, dropout_keep_prob, is_training=is_training,
                           scope='dropout6')
      net = slim.conv2d(net, 4096, [1, 1], scope='fc7', **kwargs)
      # Convert end_points_collection into a end_point dict.
      end_points = slim.utils.convert_collection_to_dict(end_points_collection)
      if global_pool:
        net = tf.reduce_mean(net, [1, 2], keep_dims=True, name='global_pool')
