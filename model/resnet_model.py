...
_NUM_MEM_CLASSES = 128
_NUM_CATE_CLASSES = 1000
...
class Model(object):
  ...
  def __init__(self, resnet_size, bottleneck, num_classes, num_filters,
               ...
               dtype=DEFAULT_DTYPE, with_cate=False):
  ...
  def __call__(
    ...
    with self._model_variable_scope():
      ...
      if skip_final_dense:
        return tf.reshape(inputs, [-1, 7 * 7 * self.final_size])
      ...
      if self.with_cate:
        # TODO: make this number of categories changable
        cate_output = tf.layers.dense(
            inputs=inputs, units=_NUM_CATE_CLASSES,
            kernel_initializer=tf.contrib.layers.variance_scaling_initializer(),
            name='cate_dense')
        ENDING_POINTS.append(cate_output)
      inputs = tf.layers.dense(inputs=inputs, units=self.num_classes)
      inputs = tf.identity(inputs, 'final_dense')
      if not get_all_layers:
        if not self.with_cate:
          return inputs
        else:
          return inputs, cate_output
      else:
