...
def main():
    ...
    tfr_dir = os.path.join(DATA_DIR, 'tfrs')
    os.system('mkdir -p %s' % tfr_dir)

    train_metas = build_all_tfrs_from_folder(
            os.path.join(args.img_folder, 'train'),
            1024,
            os.path.join(tfr_dir, 'train-%05i-of-%05i'), 
            args.random_seed)
    train_labels = [_meta[2] for _meta in train_metas]
    np.save(
            os.path.join(DATA_DIR, 'all_label.npy'), 
            np.asarray(train_labels))

    build_all_tfrs_from_folder(
            os.path.join(args.img_folder, 'val'),
            128,
            os.path.join(tfr_dir, 'validation-%05i-of-%05i'))
